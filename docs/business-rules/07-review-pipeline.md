...
Its own doc comment states it mirrors "WF-PR-REVIEWER `re-review.md`" (`src/CodeFlow.App/Review/ReviewMemory.cs`) — one
of the four citations to that external document across this codebase (`13-cross-language-contracts.md`
"An upstream document this contract depends on is not available"). That document is not in this
tree; where its reasoning is not independently recoverable from source comments, this section
