...
export function CodeSnippetPanel({ tabId }: { tabId: string }) {
  ...
  const tab = useApiTabsStore((s) => s.openTabs.find((candidate) => candidate.id === tabId) ?? null);
  const settings = useApiSettingsStore((s) => s.settings);
  const cookies = useApiCookieStore((s) => s.cookies);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void resolveRequest(draft, getVariableContext(collectionId), getAuthChainForTab(tabId), settings, cookies)
        .then((resolved) => {
          if (cancelled) return;
  ...
  if (collapsed) {
    return (
      <div className={`flex h-full w-9 shrink-0 flex-col items-center gap-2 py-2 ${CARD}`}>
        <IconButton
          label="api.snippet.expand"
          icon={PanelRightOpen}
          onClick={() => toggleCollapsed(false)}
        />
        <Code2 size={14} className="text-[var(--cf-text-muted)]" />
        <span className="mt-1 whitespace-nowrap text-badge text-[var(--cf-text-muted)] [writing-mode:vertical-rl]">
          {t("api.snippet.title")}
        </span>
      </div>
    );
    ...
    <>
      ...
      >
        ...
        <div className="flex shrink-0 items-center gap-1.5 px-2 py-1.5">
          ...
          <IconButton
            label="api.snippet.copy"
            icon={copied ? Check : Copy}
            disabled={!code}
            className={`shrink-0${copied ? " !text-[var(--cf-success)]" : ""}`}
            onClick={copy}
          />
          {copied && <span className="shrink-0 text-badge text-[var(--cf-success)]">{t("api.snippet.copied")}</span>}
        </div>

        <div className="min-h-0 flex-1">
          {error !== null ? (
            <p className="p-3 text-ui text-[var(--cf-danger)]">{t("api.snippet.failed", { error })}</p>
          ) : code === null ? null : code === "" ? (
            <p className="p-3 text-ui text-[var(--cf-text-muted)]">{t("api.snippet.unsupported")}</p>
          ) : (
            <Editor
              height="100%"
    ...
    <div ref={wrapperRef} className="relative shrink-0">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-56 space-y-2.5 rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface-raised)] p-2.5 shadow-[var(--cf-shadow)]">
          <label className="flex cursor-pointer items-center gap-2 text-ui text-[var(--cf-text)]">
          ...
          <div className="space-y-1">
            <span className="text-badge text-[var(--cf-text-muted)]">{t("api.snippet.indent")}</span>
            <Select
              size="sm"
              value={indentId}
  ...
  return (
    <>
      <button
        ...
        className={`flex min-w-0 flex-1 items-center justify-between gap-2 rounded-md border bg-[var(--cf-surface)] px-2 py-1 text-left text-ui outline-none ${
        ...
        createPortal(
          ...
          >
            <input
              ...
              onKeyDown={onKeyDown}
              className="shrink-0 border-b border-[var(--cf-border)] bg-transparent px-2.5 py-1.5 text-ui text-[var(--cf-text)] outline-none placeholder:text-[var(--cf-text-muted)]"
            />
            <div className="min-h-0 flex-1 overflow-auto p-1">
              {matches.length === 0 ? (
                ...
                groups.map(([group, targets]) => (
                  <div key={group}>
                    <div className="px-2 pb-0.5 pt-2 text-badge font-semibold uppercase tracking-wide text-[var(--cf-text-muted)]">
                      {group}
                    </div>
                    {targets.map((item) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex cursor-pointer items-center justify-between gap-2 rounded px-2 py-1.5 text-ui ${
