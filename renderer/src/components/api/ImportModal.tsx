...
import { getCurrentWebview } from "../../lib/bridge/webview";
    ...
    <>
      {items.map((item, index) => (
        <div key={`${depth}-${index}-${item.name}`}>
          <div
            className="flex items-center gap-1.5 py-[3px] text-ui"
          ...
          >
            {item.kind === "folder" ? (
              ...
              <>
                <span className="w-[46px] shrink-0 font-mono text-badge font-semibold uppercase text-[var(--cf-accent)]">
...
export function ImportModal({ onClose }: { onClose: () => void }) {
  ...
  /**
   * File drops arrive on a native webview webview channel, not as DOM `drop` events — the native
  ...
  const runImport = async () => {
    ...
    setImporting(true);
    const store = useApiTreeStore.getState();
    ...
    try {
      ...
      for (const environment of result.environments) {
        const created = await useApiEnvironmentStore.getState().createEnvironment(environment.name);
        if (!created) continue;
        await useApiEnvironmentStore.getState().updateEnvironment({
          ...created,
  ...
  return (
    <ApiModal
      ...
      footer={
        <>
          <span className="mr-auto text-badge text-[var(--cf-text-muted)]">
            {format
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="min-w-0 flex-1 truncate text-ui text-[var(--cf-text-muted)]">
              {dropActive
            ...
            <Button variant="ghost" size="sm" onClick={() => void pick()} disabled={importing}>
        ...
        <label htmlFor={pasteId} className="mb-1 block text-badge font-medium text-[var(--cf-text-muted)]">
        ...
        <textarea
          ...
          className="mb-3 h-32 w-full resize-y rounded-md border border-[var(--cf-border)] bg-transparent p-2 font-mono text-ui leading-relaxed outline-none focus:border-[var(--cf-accent)] disabled:opacity-50"
        />
            ...
            >
              <ul className="space-y-1 rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface-raised)] p-2">
                {result.warnings.map((warning, index) => (
                  <li
                    key={index}
                    className="flex gap-1.5 text-badge leading-snug text-[var(--cf-text-muted)]"
                  >
        ...
        <div className="rounded-md border border-[var(--cf-border)] bg-[var(--cf-surface-raised)] p-2">
          {!hasSomething ? (
            <p className="p-2 text-ui text-[var(--cf-text-muted)]">
            ...
            <>
              {result?.collections.map((collection, index) => (
                <div key={`${index}-${collection.name}`} className="mb-1">
                  <div className="flex items-center gap-1.5 py-[3px] text-ui font-medium text-[var(--cf-text)]">
              ...
              {result?.environments.map((environment, index) => (
                <div
                  ...
                  className="flex items-center gap-1.5 py-[3px] text-ui text-[var(--cf-text)]"
                >
