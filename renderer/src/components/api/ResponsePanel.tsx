...
import { Editor, OVERFLOW_SAFE_OPTIONS, type OnMount } from "../../lib/monacoEditor";
...
export function ResponsePanel({ tabId }: { tabId: string }) {
  ...
  const prettyPrint = useApiSettingsStore((s) => s.settings.prettyPrint);
  const maxResponseBytes = useApiSettingsStore((s) => s.settings.maxResponseBytes);
  ...
  const saveAsExample = () => {
    const tab = useApiTabsStore.getState().openTabs.find((candidate) => candidate.id === tabId);
    if (!tab) return;
  ...
  // Tests report an outcome, everything else reports a count — hence the tone. The badge text says
  // it too ("3/5"), so the colour is emphasis and not the only carrier.
  const viewOptions = VIEW_ORDER.map((id): TabOption<ResponseView> => {
    const base = { id, labelKey: VIEW_LABELS[id] };
    if (id === "tests" && response.tests.length > 0) {
      return {
        ...base,
        badge: t("api.response.testsPassed", { passed: testsPassed, total: response.tests.length }),
        badgeTone: testsPassed === response.tests.length ? "success" : "danger",
      };
    }
    const count = countBadge(id, response);
    return count ? { ...base, badge: count.text } : base;
  });
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[var(--cf-surface)]">
      <div className="flex shrink-0 items-center gap-3 border-b border-[var(--cf-border)] px-3 py-1.5">
        ...
        <div className="flex min-w-0 items-center gap-3 text-badge text-[var(--cf-text-muted)]">
          ...
          {response.redirects.length > 0 && (
            <Tooltip label={response.redirects.join(" \u2192 ")}>
              <span>{t("api.response.redirects", { n: response.redirects.length })}</span>
            </Tooltip>
          )}
        </div>
        <div className="ml-auto flex shrink-0 items-center gap-0.5">
          <IconButton
            label="api.response.copy"
            icon={copied ? Check : Copy}
            onClick={copyBody}
            className={copied ? "!text-[var(--cf-success)]" : ""}
          />
          <IconButton
            label="api.response.save"
            icon={Download}
            onClick={() => void saveToFile()}
          />
          <IconButton
            label="api.response.saveAsExample"
            icon={BookmarkPlus}
            onClick={saveAsExample}
          />
          <IconButton
            label="api.response.clear"
            icon={Eraser}
      ...
      {response.error ? (
        ...
        <>
          <Tabs
            ...
            activeId={view}
            onSelect={setView}
            layoutId="cf-api-response"
            label={t("api.response.views")}
            className="shrink-0 overflow-x-auto border-b border-[var(--cf-border)] px-2 py-1"
          />

          <div
            className="min-h-0 flex-1 overflow-hidden"
            {...tabPanelProps("cf-api-response", view)}
          >
            {view === "pretty" &&
              (binary ? (
...
function StatusPill({ response }: { response: ApiResponse }) {
  ...
  return (
    <span
      className="shrink-0 rounded-md border px-1.5 py-0.5 text-badge font-semibold"
      style={{
        color,
...
function countBadge(view: ResponseView, response: ApiResponse): { text: string } | null {
  const count =
    view === "headers"
      ...
      : view === "cookies"
        ...
        : view === "console"
          ...
          : 0;
  return count > 0 ? { text: String(count) } : null;
}
  ...
  return (
    <div className="min-h-0 flex-1 overflow-auto p-4">
      <div className="rounded-lg border border-[var(--cf-danger)]/40 bg-[color-mix(in_oklab,var(--cf-danger)_8%,transparent)] p-3">
        <div className="flex items-center gap-2 text-body font-medium text-[var(--cf-danger)]">
          <AlertTriangle size={15} />
          {t("api.response.failed")}
        </div>
        <p className="mt-2 break-all font-mono text-badge text-[var(--cf-text-muted)]">
          {response.sent?.method} {response.sent?.url}
        </p>
        <p className="mt-2 whitespace-pre-wrap text-ui text-[var(--cf-text)]">{response.error}</p>
      </div>
    </div>
  );
}
...
function BinaryBody({ response, onSave }: { response: ApiResponse; onSave: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-8 text-center">
      ...
      <p className="text-body text-[var(--cf-text)]">
        {t("api.response.binary", { size: formatBytes(response.size_bytes) })}
      </p>
      <button
        type="button"
        onClick={onSave}
        className="flex items-center gap-1.5 rounded-md border border-[var(--cf-border)] px-2.5 py-1 text-ui text-[var(--cf-text)] hover:bg-black/[0.03] dark:hover:bg-white/[0.05]"
      >
        <Download size={13} />
        {t("api.response.save")}
      </button>
      {/* `api_save_file` writes a text payload, so the bytes can only leave as Base64 text. */}
      <p className="max-w-sm text-badge text-[var(--cf-text-muted)]">{t("api.response.binaryBase64")}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-1 px-2 py-1">
        <IconButton label="api.response.search" icon={Search} onClick={openFind} />
        <IconButton
          label="api.response.wrapLines"
          icon={WrapText}
          active={wrap}
          onClick={() => setWrap((current) => !current)}
        />
        {allNotices.length > 0 && (
          <Tooltip label={allNotices.join(" · ")}>
            <span className="ml-1 truncate text-badge text-[var(--cf-warning)]">
              {allNotices.join(" · ")}
            </span>
          </Tooltip>
        )}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 justify-end px-2 py-1">
        <IconButton
          label="api.response.copyHeaders"
          icon={copied ? Check : Copy}
          onClick={copyAll}
          className={copied ? "!text-[var(--cf-success)]" : ""}
        />
          ...
          <tbody>
            {headers.map(([key, value], index) => (
              <tr key={`${key}-${index}`} className="border-b border-[var(--cf-border)] align-top">
                ...
                <td className="break-words px-3 py-1.5 font-mono text-badge text-[var(--cf-text-muted)]">
                  {value}
                </td>
              </tr>
            ))}
  ...
  return (
    <div className="h-full overflow-auto">
      <table className="w-full text-left text-ui">
        <thead className="sticky top-0 bg-[var(--cf-surface)] text-badge uppercase tracking-wide text-[var(--cf-text-muted)]">
        ...
        <tbody>
          {response.set_cookies.map((cookie, index) => (
            <tr key={`${cookie.name}-${index}`} className="border-b border-[var(--cf-border)] align-top">
              ...
              <td className="break-all px-3 py-1.5 font-mono text-badge text-[var(--cf-text-muted)]">
                {cookie.value}
              </td>
  ...
  return (
    <div className="h-full overflow-auto py-1">
      {response.tests.map((test, index) => (
        <div
          key={`${test.name}-${index}`}
          className="flex items-start gap-2 border-b border-[var(--cf-border)] px-3 py-1.5 text-ui"
        >
          {test.passed ? (
          ...
          <div className="min-w-0 flex-1">
            ...
            {test.error && (
              <p className="mt-0.5 whitespace-pre-wrap font-mono text-badge text-[var(--cf-danger)]">
                {test.error}
              </p>
            )}
          </div>
...
function ConsoleView({ response }: { response: ApiResponse }) {
  ...
  return (
    <div className="h-full overflow-auto py-1 font-mono text-badge">
  ...
  return (
    <div className="h-full overflow-auto p-4">
      <div className="flex h-4 w-full overflow-hidden rounded border border-[var(--cf-border)]">
        {measured.map((phase) => (
          <Tooltip key={phase.key} label={`${t(phase.labelKey)} · ${formatDuration(timings[phase.key])}`}>
            <div style={{ width: `${(timings[phase.key] / scale) * 100}%`, background: phase.color }} />
          </Tooltip>
        ))}
        {remainder > 0 && (
          <Tooltip label={`${t("api.timings.other")} · ${formatDuration(remainder)}`}>
            <div style={{ width: `${(remainder / scale) * 100}%`, background: HATCH }} />
          </Tooltip>
        )}
      </div>
      ...
      <div className="mt-3 space-y-1">
        {TIMING_PHASES.map((phase) => {
          ...
          return (
            <div key={phase.key} className="flex items-center gap-2 text-ui">
              <span
        ...
        <div className="flex items-center gap-2 border-t border-[var(--cf-border)] pt-1 text-ui">
