...
import { Editor, type Monaco, type OnMount } from "../../lib/monacoEditor";
...
import { reservedChordFor } from "../../lib/shortcuts";
...
// The same indicator the tab uses while its contents are still being read, so the wait for the
// parser chunk looks like the wait for the file rather than like a different kind of nothing.
const dbmlLoading = (
...
function Breadcrumb({ path, dirty, loading }: { path: string; dirty: boolean; loading: boolean }) {
  ...
  return (
    <div className="flex h-6 shrink-0 items-center gap-0.5 overflow-hidden border-b border-[var(--cf-border)] px-3 text-badge text-[var(--cf-text-muted)]">
 ...
 */
/** The chord Monaco holds for one of this toolbar's actions, already formatted. */
function paneChord(labelKey: TranslationKey): string | undefined {
  const chord = reservedChordFor(labelKey);
              ...
              <>
                {previewKind && (
                  <div className="flex items-center gap-0.5 rounded-md border border-[var(--cf-border)] p-0.5">
                    {(
                      [
                        { mode: "code", icon: Code2, labelKey: "editor.viewCode" },
                        { mode: "split", icon: Columns2, labelKey: "editor.viewSplit" },
                        { mode: "preview", icon: Eye, labelKey: "editor.viewPreview" },
                      ] as const
                ...
                {/* The chords come from `EDITOR_RESERVED`, not from a string typed here — Monaco
                    owns them, so a change to that list has to reach the tooltip. */}
                <IconButton
                  label="codesnap.action"
                  icon={Camera}
                  disabled={activeTab.loading}
                  {...(paneChord("codesnap.action") ? { shortcutLabel: paneChord("codesnap.action")! } : {})}
                  onClick={captureSnapshot}
                />
                {onSplit && (
                  <IconButton
                    label="editor.splitRight"
                    icon={SplitSquareHorizontal}
                    {...(paneChord("editor.splitRight") ? { shortcutLabel: paneChord("editor.splitRight")! } : {})}
                    onClick={onSplit}
                  />
                )}
                {onCloseGroup && <IconButton label="editor.closeGroup" icon={X} onClick={onCloseGroup} />}
                <Button
                  variant="primary"
                  size="sm"
                  icon={Save}
