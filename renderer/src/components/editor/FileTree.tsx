...
import { flattenFileTree } from "../../lib/fileTreeFlatten";
import {
  ChevronDown,
  ChevronRight,
  ChevronsDownUp,
...
import { createDir, createFile, listDir, movePath } from "../../lib/ipc/commands";
...
function DraftRow({
  ...
  return (
    <div
      ...
      className="flex items-center gap-1.5 py-0.5 pr-2 text-body"
    >
...
function TreeNode({
  ...
  return (
    <button
        ...
        style={{ paddingLeft: treeIndent(depth) }}
        // Height is declared, not a by-product of `py-0.5` around the text — that is what makes the
        // density preference possible and the virtualizer's estimate exact.
        className={`flex h-[var(--cf-row-height)] w-full items-center gap-1.5 truncate rounded-md pr-2 text-left text-body ${
          isSelected
      ...
      {ownStatus && (
        <Tooltip label={t(fileStatusLabelKey(ownStatus))}>
          <span className="ml-auto shrink-0 text-badge font-bold uppercase" style={{ color }}>
            {ownStatus[0]}
          </span>
        </Tooltip>
      )}
...
export function FileTree({
   ...
   *
   * Pointer-driven rather than HTML5 drag-and-drop for the same reason the tab strip is: the shell's
  ...
  const rows = useMemo(
    () => flattenFileTree(childrenByDir, expanded, draft?.parent ?? null),
    [childrenByDir, expanded, draft],
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-0.5 border-b border-[var(--cf-border)] px-2 py-1">
        ...
        <IconButton label="editor.newFile" icon={FilePlus} onClick={() => startDraft("file")} />
        <IconButton label="editor.newFolder" icon={FolderPlus} onClick={() => startDraft("dir")} />
        <IconButton
          label="editor.refreshExplorer"
          icon={RefreshCw}
          pending={refreshing}
          onClick={() => void refresh()}
        />
        <IconButton label="editor.collapseAll" icon={ChevronsDownUp} onClick={collapseAll} />
      </div>
      <VirtualizedTree
        rows={rows}
        ...
        // With the rows absolutely positioned, any click that isn't on a row lands on one of
        // the two containers — both mean "the root".
        onClick={(e) => {
          const target = e.target as HTMLElement;
          if (e.target === e.currentTarget || target.dataset.cfTreesizer !== undefined) {
            setFocus({ path: "", isDir: true });
          }
        }}
        // Dropping on that same empty space moves to the repo root.
        className={treeOverDir === "" ? "ring-1 ring-inset ring-[var(--cf-accent)]" : ""}
        sizerProps={{ "data-cf-treesizer": "" }}
        renderRow={(row) =>
          row.kind === "draft" && draft ? (
            <DraftRow kind={draft.kind} depth={row.depth} onSubmit={submitDraft} onCancel={cancelDraft} />
          ) : row.kind === "empty" ? (
            <p
              style={{ paddingLeft: treeIndent(row.depth) }}
              className="text-badge text-[var(--cf-text-muted)]"
            >
              {t("editor.empty")}
            </p>
          ) : row.kind === "entry" ? (
      ...
      >
        {/* Only while the first listing is in flight: `undefined` hands rendering back to the rows. */}
        {rootEntries ? undefined : <SkeletonRows count={10} className="cf-fade-in" />}
      </VirtualizedTree>
      ...
      {treeDrag && treeOrigin && (
        <DragGhost
          ...
          x={treeOrigin.x}
          y={treeOrigin.y}
          label={treeDrag.path.split("/").pop() ?? treeDrag.path}
        />
      )}
    </div>
  );
