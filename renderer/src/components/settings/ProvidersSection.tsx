  ...
  if (command) {
    return (
      <>
        ...
        <code className="rounded bg-black/[0.06] px-1 py-0.5 font-mono text-badge dark:bg-white/[0.1]">
          {command}
        </code>
      </>
    );
      ...
      {(field) =>
        hasKey ? (
          <div className="flex items-center gap-2">
            <span
              id={field.id}
              className="flex flex-1 items-center gap-1.5 rounded-md border border-[var(--cf-border)] px-2.5 py-1.5 text-body text-[var(--cf-text-muted)]"
            >
          ...
          <div className="flex gap-1.5">
            <input
              ...
              autoComplete="off"
              spellCheck={false}
              className={`${FIELD_INPUT} flex-1 font-mono`}
            />
...
function CommandLine({ command }: { command: string }) {
  ...
  return (
    <Tooltip label={t("api.snippet.copy")}>
      <button
        // Named for what pressing it does: the command alone reads as text, not as a control.
        aria-label={`${t("api.snippet.copy")}: ${command}`}
        onClick={() => {
          void navigator.clipboard.writeText(command);
          setCopied(true);
          setTimeout(() => setCopied(false), 1400);
        }}
        className="group flex w-full items-center gap-1.5 rounded-md border border-[var(--cf-border)] bg-black/[0.03] px-2 py-1 text-left font-mono text-badge text-[var(--cf-text)] hover:border-[var(--cf-accent)] dark:bg-white/[0.05]"
      >
        <span className="min-w-0 flex-1 truncate">{command}</span>
        {copied ? (
          <Check size={11} aria-hidden className="shrink-0 text-[var(--cf-success)]" />
        ) : (
          /* Decoration on a control that already has a name — revealing it on hover only sharpens
             what is already there. */
          <Copy size={11} aria-hidden className="shrink-0 text-[var(--cf-text-muted)] opacity-0 group-hover:opacity-100" />
        )}
      </button>
    </Tooltip>
  );
...
function SetupHelp({ provider }: { provider: AiProviderOption }) {
  ...
  return (
    <div className="space-y-1.5 rounded-lg border border-[var(--cf-border)] p-2.5">
      <p className="text-body font-medium text-[var(--cf-text)]">{t("settings.setupTitle")}</p>
      {command && <CommandLine command={command} />}
      {postCommand && <CommandLine command={postCommand} />}
      <Button variant="ghost" size="sm" icon={ExternalLink} onClick={() => void openExternalUrl(url)}>
        {t("settings.setupDocs")}
      </Button>
      {command && <p className="text-badge text-[var(--cf-text-muted)]">{t("settings.setupRestartHint")}</p>}
    </div>
  );
}
...
function StatusBadge({ providerId }: { providerId: string }) {
  ...
  return status.available ? (
    <span className="flex shrink-0 items-center gap-1 rounded-full bg-[color-mix(in_oklab,var(--cf-success)_14%,transparent)] px-1.5 py-0.5 text-badge font-medium text-[var(--cf-success)]">
      <CircleCheck size={10} />
      {t("settings.providerReady")}
    </span>
  ) : (
    <span className="flex shrink-0 items-center gap-1 rounded-full bg-[color-mix(in_oklab,var(--cf-warning)_16%,transparent)] px-1.5 py-0.5 text-badge font-medium text-[var(--cf-warning)]">
      <CircleAlert size={10} />
    ...
    <div className="rounded-lg border border-[var(--cf-border)]">
      <div className="flex items-center gap-2 p-2.5">
        ...
        >
          ...
          {loaded && <span className="min-w-0 truncate text-badge text-[var(--cf-text-muted)]">{modelLabel}</span>}
        </button>

        {isDefault ? (
          <Tooltip label={t("settings.providerDefaultTitle")}>
            <span className="flex shrink-0 items-center gap-1 rounded-full bg-[var(--cf-accent-soft)] px-2 py-0.5 text-badge font-medium text-[var(--cf-accent)]">
              <Star size={10} />
              {t("settings.providerDefault")}
            </span>
          </Tooltip>
        ) : (
          <Button variant="ghost" size="sm" className="shrink-0" onClick={makeDefault}>
          ...
          {!loaded ? (
            ...
            <>
              {status && !status.available && (
                <p className="rounded-md border border-[color-mix(in_oklab,var(--cf-warning)_35%,transparent)] bg-[color-mix(in_oklab,var(--cf-warning)_10%,transparent)] px-2.5 py-2 text-body leading-snug text-[var(--cf-text)]">
              ...
              {!agentic && (
                <p className="rounded-md border border-[var(--cf-border)] bg-[color-mix(in_oklab,var(--cf-accent)_8%,transparent)] px-2.5 py-2 text-body leading-relaxed text-[var(--cf-text-muted)]">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    {!isEndpoint && (
                      <Button
                        variant="secondary"
                        icon={FolderOpen}
                        tooltip={t("settings.selectClaudeBinaryTitle")}
              ...
              {agentic && !provider.usesToolAllowlist && (
                <p className="rounded-md border border-[var(--cf-border)] px-2.5 py-2 text-body leading-relaxed text-[var(--cf-text-muted)]">
              ...
              {agentic && provider.usesToolAllowlist && (
                <div>
                  <p className="mb-1 block text-relaxed font-medium text-[var(--cf-text)]">
                    {t("settings.allowedTools")}
                  </p>
                  <p className="mb-2 text-body text-[var(--cf-text-muted)]">{t("settings.allowedToolsHint")}</p>
                  {(
                    <div className="space-y-1.5 rounded-lg border border-[var(--cf-border)] p-2.5">
                      {TOOL_OPTIONS.map((tool) => (
                        ...
                        >
                          ...
                          />
                          <span>
                            <span className="block text-body font-medium">{tool.id}</span>
                            <span className="block text-badge text-[var(--cf-text-muted)]">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {tools
                        ...
                        .map((tool) => (
                          <span
                            key={tool}
                            className="flex items-center gap-1 rounded-md bg-[var(--cf-accent-soft)] px-2 py-0.5 text-badge text-[var(--cf-accent)]"
                          >
                            {tool}
                            {/* `Trash2`, not `X`: this removes a saved item from a list, and `X`
                                dismisses a surface (icon dictionary, §II.3). It had no name at all. */}
                            <IconButton
                              label="settings.removeTool"
                              icon={Trash2}
                              variant="danger"
                  ...
                  <div className="mt-2 flex gap-1.5">
                    ...
                    <button
                      ...
                      className="rounded-md border border-[var(--cf-border)] px-2.5 text-body text-[var(--cf-text-muted)] hover:bg-black/[0.03] dark:hover:bg-white/[0.04]"
                    >
                      {t("settings.add")}
                    </button>
                  </div>
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-end">
        <button
          ...
          disabled={checking}
          className="flex items-center gap-1 text-badge text-[var(--cf-text-muted)] hover:text-[var(--cf-accent)] disabled:opacity-50"
        >
