 ...
 *
 * Served by the shell rather than the sidecar. A modal dialog belongs to the process that owns the
 * window, and the sidecar has none — the same call `pickFolder` makes. The signature is unchanged,
 * so no caller knows the difference.
 */
export const apiPickFile = async (extensions: string[]) => {
  const selected = await open({ filters: [{ name: "Files", extensions }] });
