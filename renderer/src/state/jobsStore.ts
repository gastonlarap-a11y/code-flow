...
import { listJobHistory, renameJobHistoryEntry, deleteJobHistoryEntry } from "../lib/ipc/commands";
...
interface JobsState {
   ...
   * the UI shows (`invoke` isn't cancelled by React unmounting), this just makes
...
export const useJobsStore = create<JobsState>((set, get) => ({
  ...
  run: ({ projectId, kind, label, meta = {}, task }) => {
    ...
    void task(id)
      ...
      .catch((e: unknown) => {
        // An Error's own `message`, never `String(e)` — the latter prepends `"Error: "`, which
        // breaks every sentinel the UI anchors at the start of the message (`NOTHING_TO_ANALYZE: `,
        // `STALE_REVIEW: `, `CREDENTIAL_REFUSED: `, `SELF_APPROVAL: `, `CHECKOUT_CONFLICT: `). That
        // is how a clean working tree ended up shown as a red `NOTHING_TO_ANALYZE:` banner while the
        // same refusal reloaded from `job_history` — stored raw, without the prefix — rendered as
        // the calm empty state it is meant to be.
        const message = e instanceof Error ? e.message : String(e);
        settle(
          isCancellation(message)
            ? { status: "cancelled", finishedAt: Date.now() }
            : { status: "error", error: parseClaudeError(message), finishedAt: Date.now() },
        );
      })
