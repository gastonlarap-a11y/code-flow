...
import type { PrAction, PostFindingItem } from "../lib/ipc/commands";
...
/** Mirrors `AzureException.RefusedPrefix`. One of the sentinel prefixes listed in
 * `docs/business-rules/13-cross-language-contracts.md`: the transport carries a string, so an error
 * the UI has to answer differently is marked in the string itself. */
const CREDENTIAL_REFUSED_PREFIX = "CREDENTIAL_REFUSED: ";

/** Mirrors `GitHubException.SelfApprovalPrefix` (`XLANG-013`). GitHub refuses a review on your own
 * pull request by rule, so the raw 422 — which is what this used to show — invites a retry that can
 * never succeed. */
const SELF_APPROVAL_PREFIX = "SELF_APPROVAL: ";

/** Mirrors `GitHubHost.StaleReviewPrefix` (`XLANG-014`). The pull request was pushed to after the
 * review ran, so the findings' line numbers have moved and the batch was refused rather than posted
 * onto whatever now sits at them (`BUG-REVIEW-a`). */
const STALE_REVIEW_PREFIX = "STALE_REVIEW: ";

interface PrState {
  prsByProject: Record<string, PullRequestSummary[]>;
  loadingProjectId: string | null;
  loadErrorByProject: Record<string, string>;
  /** The load failed because the host refused the saved credential, not because it was unreachable.
   * Kept apart from the message so the sidebar can offer the way out instead of a bare "Retry",
   * which would fail identically every time. */
  credentialRefusedByProject: Record<string, boolean>;
...
export const usePrStore = create<PrState>((set, get) => ({
  ...
  loadPullRequests: async (projectId) => {
    set((s) => ({
      ...
      loadErrorByProject: { ...s.loadErrorByProject, [projectId]: "" },
      credentialRefusedByProject: { ...s.credentialRefusedByProject, [projectId]: false },
    }));
    try {
      const prs = await api.listPullRequests(projectId);
      set((s) => ({ prsByProject: { ...s.prsByProject, [projectId]: prs } }));
    } catch (e) {
      // A refused credential is not a network error and must not read like one: the user has a token
      // saved, the host rejected it, and the way out is to replace it. Recognised by the same kind of
      // sentinel prefix as CHECKOUT_CONFLICT — see `AzureException.RefusedPrefix`.
      const message = String(e);
      set((s) => ({
        loadErrorByProject: {
          ...s.loadErrorByProject,
          [projectId]: message.includes(CREDENTIAL_REFUSED_PREFIX)
            ? message.replace(CREDENTIAL_REFUSED_PREFIX, "")
            : message,
        },
        credentialRefusedByProject: {
          ...s.credentialRefusedByProject,
          [projectId]: message.includes(CREDENTIAL_REFUSED_PREFIX),
        },
      }));
    } finally {
  ...
  openLinkPr: (session) => {
    useAiPanelStore.getState().showPr();
    set((s) => ({
      linkPr: session,
      selectedPr: null,
      posted: false,
  ...
  closeLinkPr: () => {
    set({ linkPr: null, posted: false });
    // Nothing left to review unless a project PR is still selected underneath.
    if (!get().selectedPr) useAiPanelStore.getState().showChat();
  },
  ...
  // Reaching a PR *is* asking to look at it, so the panel's tab follows from here rather than from
  // each of the seven call sites remembering to set it. Deselecting falls back to chat, which is
  // what the priority chain did implicitly before there was a tab to name.
  selectPr: (pr) => {
    set({ selectedPr: pr, linkPr: pr ? null : get().linkPr, posted: false });
    useAiPanelStore.getState()[pr ? "showPr" : "showChat"]();
  },
  ...
  postReview: async (target, prId, runId, items, postSummary, summary) => {
    ...
    try {
      ...
      // The prefix is stripped and the message kept, unlike `SELF_APPROVAL`: this text names both
      // commits and says to run the review again, which is exactly what the user needs to read.
      const message = String(e);
      pushErrorToast(
        message.includes(STALE_REVIEW_PREFIX) ? message.replace(STALE_REVIEW_PREFIX, "") : message,
      );
      throw e;
    } finally {
      set({ posting: false });
  ...
  actOnPr: async (target, prId, action) => {
    ...
    try {
      ...
      // The sentinel replaces the message rather than being stripped from it, unlike
      // `CREDENTIAL_REFUSED`. There the raw text still told the user something ("the host said 401");
      // here it is GitHub's JSON error envelope, which tells them nothing they can act on.
      pushErrorToast(String(e).includes(SELF_APPROVAL_PREFIX) ? translate("pr.cannotApproveOwn") : String(e));
    } finally {
